...
-- !-
takeLocker :: Cabinet -> Index -> (Cabinet, Result)
takeLocker cabinet index
    | Maybe.isNothing locker = (cabinet, Right $ "locker " ++ show index ++ " nao existe")
    | state == Used = (cabinet, Right $ "locker " ++ show index ++ " ja esta em uso")
    | otherwise = (newCabinet, Left $ "seu codigo de acesso eh " ++ code)
    where 
        locker = Map.lookup index cabinet
        (state, code) = Maybe.fromJust locker
        newCabinet = Map.insert index (Used, code) cabinet

returnLocker :: Cabinet -> Index -> Code -> (Cabinet, Result)
returnLocker cabinet index code
    | Maybe.isNothing locker = (cabinet, Right $ "locker " ++ show index ++ " nao existe")
    | state == Free = (cabinet, Right $ "locker " ++ show index ++ " nao esta em uso")
    | code /= rightCode  = (cabinet, Right "codigo de acesso invalido")
    | otherwise = (newCabinet, Left "")
    where 
        locker = Map.lookup index cabinet
        (state, rightCode) = Maybe.fromJust locker
        newCabinet = Map.insert index (Free, code) cabinet
